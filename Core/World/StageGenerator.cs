...
internal static class StageGenerator
{
    ...
    public static Stage GenerateStage(int minimumRooms, int maximumRooms)
    {
        ...
        do
        {
            ...
            //For debugging
            if (bossRoom == null) bossRoomGenerations++;
            if (rooms.Count < minimumRooms) roomGenerations++;
    ...
    private static void ConnectRooms(int maximumRooms, List<Room> rooms, List<Rectangle> groundLayer,
        List<Rectangle> waterLayer, List<Door> deadEndDoors)
    {
    ...
    private static bool CanRoomsConnect(Door exitDoor, TiledMap connectingMap)
    {
        ...
        foreach (var connectingDoor in connectingDoors)
        {
            ...
            if (connectingDoorDirection == exitDoorOpposite) return true;
        }
        ...
        switch (door.Direction)
        {
            ...
            case Direction.Up:
                for (var i = 0; i < 3; i++)
                {
                    ...
                    if (roomHeight > 8) room.ChangeTile(x - 1 + i, y + 3, 20, layer);
                }

                if (x == roomWidth - 2)
                {
                    for (var i = 0; i < 4; i++) room.ChangeTile(x + 1, i, 16, layer);
            ...
            case Direction.Left:
                ...
                for (var i = 0; i < 5; i++) room.ChangeTile(x, y + 1 - i, 13, layer);
                if (y < 4)
                {
                ...
                else
                {
                    ...
                    if (roomHeight > 7) room.ChangeTile(x + 1, y - 1, 35, layer);
                }

                break;
            case Direction.Right:

                for (var i = 0; i < 5; i++) room.ChangeTile(x, y + 1 - i, 16, layer);
            ...
            case Direction.Down:
                ...
                for (var i = 0; i < 3; i++) room.ChangeTile(x - 1 + i, y, 40, layer);

                if (x >= roomWidth - 2) room.ChangeTile(roomWidth - 1, roomHeight - 1, 42, layer);
    ...
    private static Point CalculateNewRenderPos(Door exitDoor, TiledMap connectingMap)
    {
        ...
        switch (exitDoor.Direction)
        {
            case Direction.Up:
                ...
                connectingDoorX =
                    (int) Math.Floor(connectingMapDoors.First(door => door.name == "Down").x /
                                     connectingMap.TileWidth) * connectingMap.TileWidth;
            ...
            case Direction.Down:
                ...
                connectingDoorX =
                    (int) Math.Floor(connectingMapDoors.First(door => door.name == "Up").x / connectingMap.TileWidth) *
                    connectingMap.TileWidth;
            ...
            case Direction.Right:
                ...
                connectingDoorY =
                    (int) Math.Floor(connectingMapDoors.First(door => door.name == "Left").y /
                                     connectingMap.TileHeight) * connectingMap.TileWidth;
            ...
            case Direction.Left:
                ...
                connectingDoorY =
                    (int) Math.Floor(
                        connectingMapDoors.First(door => door.name == "Right").y / connectingMap.TileHeight) *
                    connectingMap.TileWidth;
